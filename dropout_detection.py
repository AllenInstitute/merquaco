...
from merquaco.pixel_classification import PixelClassification as pxl
...
class FOVDropout:
    ...
    def find_on_tissue_fovs(transcripts: pd.DataFrame, fovs: pd.DataFrame, paths_dict: dict, force_mask: bool = False):
        ...
        Notes
        -----
        `PixelClassification.create_transcript_image()` returns the original dimensions of the transcripts image, but
        ...
        mask_x_bins, mask_y_bins = pxl.create_transcript_image(transcripts_xy)
        ...
        if not force_mask and os.path.exists(mask_path):
            ...
            mask_x_bins, mask_y_bins = pxl.create_transcript_image(transcripts_xy, image_path)
            pxl.generate_mask(ilastik_program_path, image_path, pixel_class_path, object_class_path)
